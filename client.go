...
func (c *Client) Send(nums []string, tmplCode string, params map[string]string, extCode, outId string) (string, string, error) {
	if len(nums) > maxNumbers {
		return "", "", errors.New("Too many phone numbers")
	}

	query, err := c.GenQuery(nums, tmplCode, params, extCode, outId, "JSON")
	if nil != err {
...
func (c *Client) GenQuery(nums []string, tmplCode string, params map[string]string, extCode, outId, format string) (url.Values, error) {
	...
	//log.Println(plain)
	h.Write([]byte(signPrefix + url.QueryEscape(query.Encode())))
